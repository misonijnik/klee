...
void Executor::executeAlloc(ExecutionState &state, ref<Expr> size, bool isLocal,
  ...
  if (inBounds != Solver::Validity::False) {
    ...
    if (!mo) {
      ...
      if (inBounds == Solver::Validity::True) {
...
MemoryObject *Executor::allocate(ExecutionState &state, ref<Expr> size,
  ...
  ref<Symcrete> addressSymcrete = new Symcrete(addressExpr);
  ref<Symcrete> sizeSymcrete = new SizeSymcrete(size, addressSymcrete);
  ...
  solver->setTimeout(coreSolverTimeout);
  bool success =
      solver->getMinimalUnsignedValue(state.constraints.cs(), symbolicSizesSum,
...
bool Executor::resolveMemoryObjects(
  ...
  if (state.resolvedPointers.count(address)) {
    assert(state.resolvedPointers.at(address).size() > 1);
    ...
    for (ResolutionList::iterator i = rl.begin(), ie = rl.end(); i != ie; ++i) {
      ...
      ref<Expr> notInBounds = NotExpr::create(inBounds);
    ...
    bool mayLazyInitialize =
        LazyInitialization && !isa<ConstantExpr>(baseUniqueExpr) &&
        state.resolvedPointers.count(base) == 0 &&
    ...
    if (mayLazyInitialize) {
      ...
      if (!success) {
        ...
        IDType idLazyInitialization =
            lazyInitializeObject(state, base, target, baseTargetType, size);
...
bool Executor::checkResolvedMemoryObjects(
  ...
  for (unsigned int i = 0; i < mayBeResolvedMemoryObjects.size(); ++i) {
    const MemoryObject *mo =
        state.addressSpace.findObject(mayBeResolvedMemoryObjects[i]).first;

    ref<Expr> inBounds = mo->getBoundsCheckPointer(address, bytes);
    ref<Expr> baseInBounds = ConstantExpr::create(1, Expr::Bool);
  ...
  if (!mayBeOutOfBound) {
    checkOutOfBounds = ConstantExpr::create(0, Expr::Bool);
  }
  return true;
}
...
bool Executor::collectConcretizations(
  ...
  guard = ConstantExpr::create(0, Expr::Bool);

  if (resolveConditions.size() > 0) {
    ref<Expr> excludeGuard = ConstantExpr::create(1, Expr::Bool);
    ref<Expr> selectGuard = ConstantExpr::create(0, Expr::Bool);
    for (unsigned int i = 0; i < resolveConditions.size(); ++i) {
      ref<Expr> head = resolveConditions[i];
      ref<Expr> body = ConstantExpr::create(1, Expr::Bool);
      for (unsigned int j = 0; j < resolveConditions.size(); ++j) {
        if (i != j) {
          body = AndExpr::create(body, NotExpr::create(resolveConditions[j]));
        }
      }
      excludeGuard = AndExpr::create(
          excludeGuard, OrExpr::create(NotExpr::create(head), body));
      selectGuard = OrExpr::create(selectGuard, resolveConditions[i]);
    }
    guard = AndExpr::create(excludeGuard, selectGuard);
  }
  guard = AndExpr::create(NotExpr::create(checkOutOfBounds), guard);

  for (unsigned int i = 0; i < resolvedMemoryObjects.size(); ++i) {
    if (concretizationManager->contains(state.constraints.cs(),
                                        resolveConditions[i])) {
      Assignment assigment = concretizationManager->get(state.constraints.cs(),
                                                        resolveConditions[i]);
...
void Executor::executeMemoryOperation(
  ...
  if (!checkResolvedMemoryObjects(*state, address, targetType, target, bytes,
  ...
  if (MergedPointerDereference) {
    ...
    if (!collectConcretizations(*state, resolveConditions,
                                resolvedMemoryObjects, checkOutOfBounds, guard,
...
IDType Executor::lazyInitializeObject(ExecutionState &state, ref<Expr> address,
  ...
  executeMakeSymbolic(state, mo, targetType,
                      SourceBuilder::lazyInitializationContent(address), false);
  return mo->id;
}
...
ref<Expr> Executor::makeSymbolicValue(llvm::Value *value,
  ...
  auto source =
      argument ? SourceBuilder::argument(*dyn_cast<Argument>(value), 0)
               : SourceBuilder::instruction(*dyn_cast<Instruction>(value), 0);
