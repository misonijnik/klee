...
void *AddressManager::allocate(ref<Expr> address, uint64_t size) {
  ...
  if (size > maxSize) {
    newMO = nullptr;
    objects[size] = newMO;
  } else if (sizeLocation == objects.end() || !sizeLocation->second) {
    ...
    newMO =
        memory->allocate(newSize, mo->isLocal, mo->isGlobal,
                         mo->isLazyInitialized, mo->allocSite, mo->alignment,
                         mo->addressExpr, mo->sizeExpr, mo->timestamp, mo->id);
  } else {
