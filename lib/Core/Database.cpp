...
void Database::create_schema() {
  ...
  sql_create = "CREATE TABLE constr"
               ...
               "expr_id INTEGER REFERENCES expr(id),"
               "summary_id INTEGER REFERENCES lemma(id))";
  finalize(sql_create, st);
  ...
  sql_create = "CREATE TABLE parent"
               "(id INTEGER NOT NULL PRIMARY KEY,"
               "child_id INTEGER REFERENCES array(id),"
               "parent_id INTEGER REFERENCES array(id))";
  finalize(sql_create, st);
}
...
void Database::parent_write(int64_t child, int64_t parent) {
  std::string sql = "INSERT INTO parent (child_id, parent_id)"
                    "VALUES (" +
...
void Database::constraint_write(int64_t expr, int64_t summary) {
  std::string sql = "INSERT INTO constr (expr_id, summary_id)"
                    "VALUES (" +
...
void Database::arraymap_write(int64_t array, int64_t expr) {
  std::string sql = "INSERT INTO arraymap (array_id, expr_id)"
                    "VALUES (" +
