...
class CFGDistance {
  ...
  using functionDistanceMap =
      std::unordered_map<KFunction *, std::unordered_map<KFunction *, unsigned>>;
  using functionDistanceList =
      std::unordered_map<KFunction *, std::vector<std::pair<KFunction *, unsigned>>>;
...
public:
  ...
  const std::unordered_map<KBlock *, unsigned int> &getBackwardDistance(KBlock *kb);
  const std::vector<std::pair<KBlock *, unsigned int>> &
  getSortedDistance(KBlock *kb);
  const std::vector<std::pair<KBlock *, unsigned int>> &
  getSortedBackwardDistance(KBlock *kb);

  const std::unordered_map<KFunction *, unsigned int> &getDistance(KFunction *kf);
  const std::unordered_map<KFunction *, unsigned int> &getBackwardDistance(KFunction *kf);
