...
#define memset_sp(dest, destMax, c, count)  /*lint -save -e506 -e1055 -e650 -e778 -e802 */ (__builtin_constant_p((count)) ? (MEMSET_SM((dest), (destMax),  (c), (count))) :  \
    ...
    **************************************************************************/
    ...
    /**************************************************************************
    * 函数名称： fwscanf_s_sp
    * 功能描述： 从安全文件流中读取格式化数据信息（Unicode字符）
    * 参数说明： (IN)
    *            pSecStr 安全文件流
    *            format 格式化字符串
    *             ... 
    ...
    *            (OUT)
    *             
    * 返 回 值： 安全文件流对象，如果打开失败，返回NULL
    * 其它说明： 安全文件流只能为只读，无法对其进行写操作。
    * 操作示例：
    ****************************************
    *  int a;
    ...
    **************************************************************************/
    ...
    /**************************************************************************
    * 函数名称： sec_fopen_op_ex
    * 功能描述： 根据文件对象建立安全文件流
    * 参数说明： (IN)
    fp 文件对象
    *            (OUT)
    *             
    * 返 回 值： 安全文件流对象，如果建立失败，返回NULL
    * 其它说明： 
    * 修改历史：
    1、日期;
    **************************************************************************/
