...
// RUN: %klee --output-dir=%t.klee-out --run-in-dir=%t.klee-out-tmp --use-guided-search=false --search=random-state --libc=uclibc --posix-runtime --exit-on-error %t.bc --sym-files 1 1 > %t1.log
// RUN: %clang -D_FILE_OFFSET_BITS=64 %s -emit-llvm %O0opt -c -g  -o %t.bc
// RUN: rm -rf %t.klee-out %t.klee-out-tmp
// RUN: %gentmp %t.klee-out-tmp
// RUN: %klee --output-dir=%t.klee-out --run-in-dir=%t.klee-out-tmp --use-guided-search=false --search=random-state --libc=uclibc --posix-runtime --exit-on-error %t.bc --sym-files 1 1 > %t2.log
