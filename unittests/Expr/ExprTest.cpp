...
TEST(ExprTest, ConcatExtract) {
  ArrayCache ac;
  const Array *array = ac.CreateArray("arr0", 256, SourceBuilder::makeSymbolic());
  ref<Expr> read8 = Expr::createTempRead(array, 8);
  const Array *array2 = ac.CreateArray("arr1", 256, SourceBuilder::makeSymbolic());
...
TEST(ExprTest, ExtractConcat) {
  ArrayCache ac;
  const Array *array = ac.CreateArray("arr2", 256, SourceBuilder::makeSymbolic());
  ref<Expr> read64 = Expr::createTempRead(array, 64);

  const Array *array2 = ac.CreateArray("arr3", 256, SourceBuilder::makeSymbolic());
  ref<Expr> read8_2 = Expr::createTempRead(array2, 8);
...
TEST(ExprTest, ReadExprFoldingBasic) {
  ...
  const Array *array = ac.CreateArray("arr", size, SourceBuilder::constant(), &Contents[0],
                                      &Contents[0] + size);
...
TEST(ExprTest, ReadExprFoldingSymbolicValueUpdate) {
  ...
  ref<Expr> index = ConstantExpr::create(0, Expr::Int32);
  UpdateList ul(array, 0);
  const Array *array2 = ac.CreateArray("arr2", 256, SourceBuilder::makeSymbolic());
